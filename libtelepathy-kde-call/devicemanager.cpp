...
void DeviceManagerPrivate::detectDevices()
{
    ...
    Q_FOREACH(const Solid::Device & solidDevice, solidAudioDevices) {
    ...
    Q_FOREACH(const Solid::Device & solidDevice, solidVideoDevices) {
        addDevice(solidDevice);
    }
    ...
    if ( solidDevice.is<Solid::AudioInterface>() ) {
        ...
        if ( solidAudioDevice->deviceType() & Solid::AudioInterface::AudioInput )
        {
            ...
            case Solid::AudioInterface::Alsa:
            {
                ...
                Q_EMIT q->devicesChanged(DeviceManager::AudioInput);
                break;
            }
        ...
        if ( solidAudioDevice->deviceType() & Solid::AudioInterface::AudioOutput )
        {
            ...
            case Solid::AudioInterface::Alsa:
            {
                ...
                Q_EMIT q->devicesChanged(DeviceManager::AudioOutput);
                break;
            }
            case Solid::AudioInterface::OpenSoundSystem:
        ...
        if ( solidVideoDevice->supportedProtocols().contains("video4linux") ) {
            ...
            if ( solidVideoDevice->supportedDrivers("video4linux").contains("video4linux2") ) {
                ...
                Q_EMIT q->devicesChanged(DeviceManager::VideoInput);
    ...
    for (uint i=0; i<3; i++) {
        ...
        for(; it != m_availableDevices[i].end(); ++it) {
            if ( (*it).udi() == udi ) {
                ...
                Q_EMIT q->devicesChanged(static_cast<DeviceManager::DeviceType>(i));
                return;
            }
        }
    }
...
void DeviceManager::setCurrentDeviceForType(DeviceType type, const Device & device)
{
    ...
    Q_EMIT currentDeviceChanged(type);
}
...
void DeviceManager::loadConfig(const KConfigGroup & config)
{
    ...
    for(int i=0; i<4; i++) {
        bool found = false;
        Q_FOREACH(const Device & availableDevice, d->m_availableDevices[i]) {
...
QGst::ElementPtr DeviceManager::newAudioInputElement()
{
    Device device = d->m_currentDevices[AudioInput];
    QGst::ElementPtr element = QGst::ElementFactory::make(device.driver());
    if ( element.isNull() ) {
...
QGst::ElementPtr DeviceManager::newAudioOutputElement()
{
...
QGst::ElementPtr DeviceManager::newVideoInputElement()
{
...
QGst::Ui::VideoWidget *DeviceManager::newVideoWidget(QWidget *parent)
{
    ...
    QGst::Ui::VideoWidget *widget = new QGst::Ui::VideoWidget(parent);
    widget->setVideoSink(element);
    return widget;
}
